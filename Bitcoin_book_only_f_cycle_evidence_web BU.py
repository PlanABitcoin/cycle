...
def make_website(table, current_block_val, blocks_per_segment):
   ...
   # ==========================================
    ...
    marker_index = int(exact_x_pos)
    marker_index = max(0, min(marker_index, len(segments) - 1))

    marker_segment = segments[marker_index]

    # location inside current segment
    frac = exact_x_pos - marker_index - 0.5

    # convert to pixels (adjust 100-140 if needed)
    xpix = frac * 300
    ...
    exact_x_pos = current_block_val / blocks_per_segment
    marker_percent = 100 * exact_x_pos / SEGMENTS

    current_marker = Div(text=f"""
    <div style="
        position: relative;
        width: 92%;
        max-width: 1250px;
